    ...
    <div class="left">
        ...
        <div class="right">
            ...
            <div class="right-subtilte">
                <h5 class="text-secondary">Classe ({{$nb_classe}})</h5>
                <div>
                <button class="btn btn-info" id="openAddForAdmin">Ajouter une classe</button>
            ...
            <div class="right-content-tilte">
                ...
                <div class="right-content-subtitle">
                    ...
                    <div class="right-content-subtitle-row">
                        <div class="col  text-center p-2 text-secondary">{{$classes->id}}</div>
                        <div class="col  text-center p-2">{{$classes->nom}}</div>
                        <div class="col  text-center py-2 p-0">
                            <button class="btn-action btn text-white">Actions</button></div>
                    </div>
                    @endforeach
                    @else
...
<div id="addForAdmin" class="modal">
        <div class="modal-content">
            ...
            <h5 class="d-flex justify-content-center pb-4">Classe</h5>
            <form id="dateForm" method="POST" action="{{ route('classe.storeAdmin') }}">
                @csrf
                <div class="form-group">
                    <label for="startYear">Nom</label>
                    <input type="text" id="startYear" name="nom" required class="form-control">
                </div>
    ...
    <div id="dateModal" class="modal">
        <div class="modal-content">
            <span class="close-btn d-flex justify-content-end">&times;</span>
            <h5 class="d-flex justify-content-center pb-4"> Classe</h5>
            <form id="dateForm" method="POST" action="{{ route('classe.store') }}">
                @csrf
                <div class="form-group">
                    <label for="startYear">Nom</label>
                    <input type="text" id="startYear" name="nom" required class="form-control">
    ...
    <div id="assignedModal" class="modal">
        <div class="modal-content">
            <span class="close-btn d-flex justify-content-end">&times;</span>
            <h5 class="d-flex justify-content-center pb-4">Classe par rapport à une année</h5>
            <form id="dateForm" method="POST" action="{{ route('classeanneescolaire.store') }}">
                @csrf
                <div class="form-group">
                    <label for="startYear">Année scolaire</label>
                    <select name="id_annee_scolaire" class="form-control" required>
                        <option value="" disabled selected>Choisir une année</option>
                        @foreach ($annee_scolaire as $annee_scolaireItem)
                            <option value="{{ $annee_scolaireItem->id }}">{{Carbon\Carbon::parse($annee_scolaireItem->date_de_debut)->format('Y')}}-{{Carbon\Carbon::parse($annee_scolaireItem->date_de_fin)->format('Y')}}</option>
                        @endforeach
                    </select>
                    <br>
                    <label for="startYear">Nom de la classe</label>
                    <select name="id_classe" class="form-control" required>
                        <option value="" disabled selected>Choisir une classe</option>
                        @foreach ($classe as $classeItem)
                            <option value="{{ $classeItem->id }}">{{ $classeItem->nom }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="d-flex justify-content-end pt-2">
