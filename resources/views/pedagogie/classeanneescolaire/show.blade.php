        ...
        <div class="right">
            ...
            <div class="right-subtilte">
                ...
                <div class="d-flex">
                    ...
                    <a href="{{ url()->previous() }}">
                    <h5 class="text-secondary px-1">{{Carbon\Carbon::parse($anneescolaire->date_de_debut)->format('Y')}}-{{Carbon\Carbon::parse($anneescolaire->date_de_fin)->format('Y')}} \</h5>
                    </a>
                    <h5 class="text-secondary">{{$classe->nom}}</h5>
                </div>
                    @if(session('error'))
                ...
                <div class="d-flex">
                <button class="btn btn-info mx-1" id="openModalBtn">Ajouter un étudiant</button>
                <button class="btn btn-dark" id="addEtudiant">Assigner un cours</button>
                </div>
            </div>
      ...
      <div class="right-content">
            <div class="right-content-tilte">
                ...
                <div class="right-content-subtitle">
                    <p hidden>{{$nb_etudiant = 0}}</p>
                    @if($nb_etudiants >= 1)
                    @foreach ($etudiant as $cas)
                    <p hidden>{{$nb_etudiant = 1 + $nb_etudiant}}</p>
                    <div class="right-content-subtitle-row">
                        <div class="col  text-center p-2 text-secondary">{{$nb_etudiant}}</div>
                        <div class="col  text-center p-2">{{$cas->personne->name}}</div>
                        <div class="col  text-center p-2">{{$cas->personne->first_name}}</div>
                        <div class="col  text-center p-2">{{$cas->personne->email}}</div>
                        <div class="col  text-center p-2">{{$cas->date_de_naissance}}</div>
                    ...
                    @else
                        <div class="d-flex justify-content-center text-secondary py-2">
                            Aucun étudiant n'a été inscrit dans cette classe !
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    window.onclick = function(event) {
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
    // Get the modal
    var modal = document.getElementById('addEtudiantPopUp');

    // Get the button that opens the modal
    var btn = document.getElementById('addEtudiant');
    ...
    <div id="dateModal" class="modal">
        <div class="modal-content">
            ...
            <h5 class="d-flex justify-content-center">Etudiant</h5>
            <form id="dateForm" method="POST" action="{{ route('etudiant.storeAdminCas' , ['id' => $classe->id , 'id_annee' => $anneescolaire->id]) }}">
                @csrf
    ...
    <div id="addEtudiantPopUp" class="modal">
        <div class="modal-content">
            <span class="close-btn d-flex justify-content-end">&times;</span>
