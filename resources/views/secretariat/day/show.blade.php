<!-- Le jour est {{$day->anneescolaires->date_de_fin}}
        ...
        <div class="right">
            ...
            <div class="right-subtilte">
                ...
                <div class="d-flex">
                    ...
                    <h5 class="text-secondary">Année Scolaire\</h5>
                    </a>
                    <a href="{{ url()->previous() }}">
                    <h5 class="text-secondary px-1">{{Carbon\Carbon::parse($day->anneescolaires->date_de_debut)->format('Y')}}-{{Carbon\Carbon::parse($day->anneescolaires->date_de_fin)->format('Y')}} \</h5>
                    </a>
                    <a href="{{ url()->previous() }}">
                    <h5 class="text-secondary px-1">Jour : {{Carbon\Carbon::parse($day->date)->format('d-m-y')}} \</h5>
                    </a>
                    <h5 class="text-secondary">Heure</h5>
                </div>
                    @if(session('error'))
                ...
                <button class="btn btn-info" id="openModalBtn">Ajouter un Jour</button>
            </div>

      <div class="right-content">
            <div class="right-content-tilte">
                <div class="col  text-center p-2"><h6>ID</h6></div>
                <div class="col  text-center p-2"><h6>Heure de debut</h6></div>
                <div class="col  text-center p-2"><h6>Heure de fin</h6></div>
                <div class="col  text-center p-2"><h6>Actions</h6></div>                    
            </div>
            
                <div class="right-content-subtitle">
                <p hidden>{{$nb_day_hours = 0}}</p>
                    @if($day_hour->count() >= 1)
                    @foreach ($day_hour as $jour)
                    <p hidden>{{$nb_day_hours = 1 + $nb_day_hours}}</p>
                    <div class="right-content-subtitle-row">
                        <div class="col  text-center p-2 text-secondary">{{$nb_day_hours}}</div>
                        <div class="col text-center p-2">{{Carbon\Carbon::parse($jour->hours->heure_de_debut)->format('H:i')}}</div>
                        <div class="col text-center p-2">{{Carbon\Carbon::parse($jour->hours->heure_de_fin)->format('H:i')}}</div>
                        <div class="col  text-center py-2 p-0">
                        <!-- <a href="{{route('day.show' , ['id' => $jour->days->id])}}"> -->
                    ...
                    @else
                        <div class="d-flex justify-content-center text-secondary py-2">
                            Aucun jour n'a été enregistrer !
                        </div>
                    @endif
                </div>
            </div>
...
<script>
    ...
    <div id="dateModal" class="modal">
        <div class="modal-content">
            ...
            <h5 class="d-flex justify-content-center pb-2">Heures par rapport au jour</h5>
            <form action="{{ route('hours.store', ['id' => $day->id]) }}" method="POST">
    @csrf
            ...
            @foreach($hours as $hour)
                <tr>
                    <td class="text-center">
                        <input class="form-check-input" type="checkbox" name="hours[]" value="{{ $hour->id }}" id="hour{{ $hour->id }}">
                    </td>
                    <td class="text-center">{{ $hour->id }}</td>
                    <td class="text-center">{{ $hour->heure_de_debut }}</td>
                    <td class="text-center">{{ $hour->heure_de_fin }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
